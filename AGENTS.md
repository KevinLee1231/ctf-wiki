...
本仓库是单人自用仓库，只使用 `main` 维护：

- 所有维护提交直接在 `main` 上创建，禁止新建本地或远端功能分支、归档分支及其它开发分支；
- 不创建或提交 Pull Request，也不按多人协作流程保留待审分支；
- 每次写入前先确认当前分支为 `main`，并检查工作树、`origin/main` 与远端最新状态；
- 历史远端分支可以作为只读来源合并进 `main`，但删除远端分支仍需用户明确授权；
- 自动推送仍然禁止；只有用户明确要求推送时，才可直接推送 `main`，且推送前后必须核对 `HEAD`、`origin/main`、ahead/behind、工作树状态与远端 `refs/heads/main`。
