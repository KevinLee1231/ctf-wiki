...
这道题主要受到 这篇邮件的启发，非常感谢 @tr3e 师傅！

在新版本的 kernel 中，不论是 verifier 还是 ALU sanitizer 都加强了检测，上一题中提到的利用完全失效，但是通过 bpf_skb_load_bytes  函数仍然可以实现利用。

题目仍然利用“运行时值与 verifier 认知不一致”的寄存器，但不再走上一题的 map metadata OOB 路线，而是借 `bpf_skb_load_bytes` 把过长 packet 数据拷贝到 eBPF 栈上，直接形成 eBPF 栈溢出。外部邮件/讨论的关键信息可以概括为：即便 ALU sanitizer 限制了显式指针偏移，只要 helper 的长度参数被 verifier 错判为安全，helper 内部的大长度拷贝仍然可能变成栈破坏原语。
...
如果我们可以让 len 大于栈上 buf 的长度，就可以直接栈溢出。由于添加的漏洞可以让我们获得一个运行时值为 1，而 verifier 认定为 0 的寄存器，所以可以很容易的指定一个很长的 len，并且骗过

verifier。

唯一的问题是 leak，也许可以通过溢出修改 bpf 栈上的指针变量实现任意地址读，但是笔者在调试时发现新版本内核在 ebpf 程序 crash（如 0 地址访问）时并不会造成内核崩溃（因为这属于 “soft panic”，当 /proc/sys/kernel/panic_on_oops 值为 0 时 soft panic 并不会直接 panic。似乎在默认情况下其值就是 0，如 Ubuntu 20.04。在 ctf 的 kernel pwn 题中，可能由于不希望被通过 crash 打印日志的方法leak，一般都会在 qemu 启动项里通过 oops = panic 来让 soft panic 也直接造成 kernel 的重启），还会打出一些地址信息，笔者就直接通过这种方式完成 leak 了。
...
- 核心技巧：利用 verifier 错判构造超长 `len`，让 `bpf_skb_load_bytes(skb, offset, to, len)` 向 eBPF 栈缓冲区拷贝超过预期长度的数据。
- 泄露方式：如果目标环境没有设置 `oops=panic`，eBPF 程序崩溃属于 soft panic，`panic_on_oops=0` 时不会直接重启，日志中可能打印内核地址，可用来计算 KASLR 偏移。
- 提权路线：拿到内核偏移后，用栈溢出布置 ROP，调用 `commit_creds(init_cred)`，再通过 `swapgs_restore_regs_and_return_to_usermode` 返回用户态执行 shell。
- 复用要点：新内核的 ALU sanitizer 会封堵很多显式 OOB，但 helper 调用仍需重点检查参数上界，尤其是长度参数和栈目标地址的组合。
