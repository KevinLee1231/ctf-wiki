...
题目是 JavaScriptCore Pwn。补丁移除了 DFG JIT 中 `ValueAdd` 的 clobberize 处理，并放宽了 `jsCast` 中部分 x86 平台校验。利用链通过 JIT 类型混淆构造 `addrof/fakeobj`，泄露随机化后的 `structure id`，绕过 Gigacage 后建立任意地址读写，最后写入 wasm RWX 内存执行 shellcode。

## 解题过程

补丁核心有两个点：

1. 移除 `dfg jit` 中 `ValueAdd` 操作的 `clobberize`。
2. 移除 `jsCast` 中部分 x86 平台校验。

借助第一个点，可以在 `dfg jit` 中构造 `type confusion`（使用 `ValueAdd` 引入副作用），将 `ArrayWithDouble` 在一次 `ValueAdd` 中转为 `ArrayWithContiguous`，而编译后的代码仍将其当作 `ArrayWithDouble` 处理，进而构造 `addrof` 与 `fakeobj` 原语。

利用第二点移除的检查，可以使用 `static ALWAYS_INLINE JSValue getByVal(VM& vm, JSValue baseValue, JSValue subscript)` 漏出随机化后的 `structure id`，并据此构造正确的 `fakeObj` 与 `makeObj`。

随后需要绕过 `gigacage`。构造 `fakeObj` 后可访问 `victim->butterfly - 0x10`（可控区域），从而获得任意地址读写能力。再构造 `shared butterfly`，获取新一组 `addrof` 与 `fakeobj` 原语。

最终利用任意读写原语创建 wasm 对象，定位 `rwx` 段地址并写入预置 shellcode。
...
- 核心技巧：利用 DFG JIT 未正确 clobberize 的副作用制造 `ArrayWithDouble` 与 `ArrayWithContiguous` 类型混淆。
- 识别信号：JSC 补丁涉及 `ValueAdd`、DFG、`clobberize`、`jsCast` 或 structure id 检查时，应重点寻找 JIT 假设与运行时对象类型不一致的路径。
- 复用要点：浏览器 Pwn WP 要保留 primitive 的构造顺序：类型混淆到 `addrof/fakeobj`，泄露 structure id，构造 fake object，绕过 Gigacage，任意读写，wasm RWX 执行。
