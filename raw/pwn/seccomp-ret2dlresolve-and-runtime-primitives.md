# CTF Pwn - Seccomp, ret2dlresolve and Runtime Primitives
...
- [运行时短案例技巧族：类型、索引、格式化与 GC 原语](#运行时短案例技巧族类型索引格式化与-gc-原语)
...
**See also:** heap-houses-unlink-and-tcache.md — House of Apple 2, House of Einherjar, House of Orange/Spirit/Lore/Force, heap grooming, custom allocator exploitation (nginx, talloc), classic unlink, musl libc heap, tcache stashing unlink
---
...
See ret2csu-dynelf-and-shellcode.md for full details and code examples.
...
**关键结论：** Both structs must be the same size for tcache to reuse the chunk.
...
**模式（Santa's Christmas Calculator）：** Off-by-one in instruction encoding causes misaligned machine code.
...
**BF JIT unbalanced bracket:** Unbalanced `]` pops tape address (RWX) from stack → write shellcode to tape with `+`/`-`, trigger `]` to jump to it. See seccomp-ret2dlresolve-and-runtime-primitives.md.


---
## Esoteric Language GOT Overwrite

**模式（Pikalang）：** Brainfuck/Pikalang interpreter with unbounded tape allows arbitrary memory access.
...
**关键结论：** Unbounded tape = arbitrary read/write primitive relative to buffer base.
...
**模式（Santa's Base Converter）：** Number stored as string in different bases has different lengths.
...
**模式（Christmas Trees）：** Imbalanced binary tree causes stack buffer underallocation.
...
For comprehensive kernel exploitation techniques, see linux-kernel-exploit-basics.md. 速查：
...
**关键结论：** Tiny shellcode budgets can still leak a full flag if you turn the loop / crash distinction into a 1-bit channel. Any operation that hangs on one branch and crashes on the other works — `hlt`, page faults, or explicit infinite loops.

**参考：** hxp CTF 2018 — yunospace, writeup 12570
...
**关键结论：** Windows NT API has several "dump register state" helpers intended for unwinding and exception handling. They behave as deterministic info-leak primitives for exploitation because they copy `RSP` verbatim into user memory with no randomisation.

**参考：** Insomnihack 2017 — winworld, writeup 12876
...
**关键结论：** IEEE 754 doubles are lossless integer containers whenever the exponent field is fixed at `bias + 52`. Any "you can only write N doubles" primitive is equivalent to "you can write N×6 bytes of raw data", as long as you control the exponent bits. Works identically for 32-bit floats (`bias + 23`) and long doubles.

**参考：** Kaspersky Industrial CTF 2018 — doubles, writeups 12324, 12326
...
**关键结论：** 32-bit PIE on many distros emits `ET_DYN` with a stock `mmap_base` of `0x56555000` because `brk_randomization` and ASLR entropy are minimal. If `info proc mappings` shows the same base across multiple runs (in the challenge container or in gdb with `set disable-randomization on`), treat the "random" base as a constant. Always enumerate map bases before assuming a leak is required — the same trick applies to stacks started under `ulimit -s unlimited` (base becomes `0x7fff_f000` deterministically).

**参考：** TAMUctf 2019 — pwn2, writeup 13423


---

## 运行时短案例技巧族：类型、索引、格式化与 GC 原语
...
**模式（Encodinator）：** Base85-encoded input in RWX memory passed to `printf()`. Write shellcode to RWX region, overwrite `.fini_array[0]` via format string `%hn` writes. Use convergence loop for base85 argument numbering. See seccomp-ret2dlresolve-and-runtime-primitives.md.
...
**模式（Hashchain）：** Brute-force MD5 preimages with `eb 0c` prefix (jmp +12) to skip middle bytes; bytes 14-15 become 2-byte i386 instructions. Build syscall chains from gadgets like `31c0` (xor eax), `cd80` (int 0x80). See seccomp-ret2dlresolve-and-runtime-primitives.md for C code and v2 technique.
