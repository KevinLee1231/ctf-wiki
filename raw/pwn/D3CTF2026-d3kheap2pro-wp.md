...
题目提供了 Linux 7.1.4 内核、QEMU 启动脚本、rootfs 和一个名为
...
WP 生成时，最初提供的远端实例已经没有可用后端：TLS 可以握手，但
guest shell 会在收到第一条命令后断开。因此上面的值是完整利用在原始
本地环境中以 `ctf` 用户提权后读取到的附件 flag；远端值需要在新的实例
上用 `solve.py` 再确认。
...
这些约束共同构成了 `d3kheap2pro` 相比旧版题目的主要修复与绕过点。

赛后材料披露的 OOM root shell 与 QEMU TCG 越界均是旁路非预期：它们分别利用启动配置和宿主模拟器，完全跳过上述 sheaf 链。revenge 修复这些旁路后，模块的 double free 与 Linux 7.1 allocator 行为仍是题目的决定性主障碍。
