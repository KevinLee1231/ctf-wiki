# CTF Forensics - Video, Document and Media Stego

See also: audio-frequency-and-archive-stego.md for audio steganography (FFT frequency domain, DTMF, SSTV, LSB audio, musical notes, metadata encoding, waveform binary, spectrogram QR) and whitespace/archive encoding.

## 阅读定位

- 本卷是高级隐写的 **视频 / 文档 / 图像补卷**。
- 如果题目还停留在音频、频域或基础高级隐写，不要跳过 audio-frequency-and-archive-stego.md 直接来这里。
...
- [RTF Ignorable-Destination Custom-Tag Extraction (VolgaCTF 2013)](#rtf-ignorable-destination-custom-tag-extraction-volgactf-2013)
...
**关键结论：** Content obscured by motion, noise, or rapid changes across frames becomes visible when averaged. Extract frames with `ffmpeg -i video.mp4 frames/%04d.png` first. Works for hidden QR codes, text, and watermarks.

---

## JPEG XL TOC Permutation Steganography (BSidesSF 2026)

**模式（image-progress）：** JPEG XL's Table of Contents (TOC) supports a permutation field that reorders how AC groups (progressive scan tiles) are stored in the file. The convergence order during progressive decoding — which 256x256 tiles appear first as you truncate the file at increasing offsets — encodes the flag.
...
**关键结论：** JPEG XL's TOC permutation is a legitimate feature for progressive rendering optimization (prioritize important image regions). As a steganographic channel, it's invisible — the fully decoded image looks identical regardless of permutation. The hidden data is only revealed by observing the progressive convergence order, which requires truncating the file at multiple points.
...
**关键结论：** Arnold's Cat Map is periodic with period dividing `3*N` for most image sizes. Iterating the forward transform eventually restores the original. For large images, compute the period analytically via `lcm` of matrix eigenvalue orders in `Z/NZ` rather than brute-forcing all iterations.
...
**关键结论：** JPEG decoders stop at the FFD9 (End of Image) marker and ignore trailing bytes. In MJPEG streams, each frame is a complete JPEG — appending 1+ extra bytes after each frame's FFD9 creates a covert channel invisible to video players.
...
**关键结论：** Standard LSB tools (zsteg, stegsolve) fail with non-sequential pixel patterns. The Stegano library supports custom generators; always check EXIF metadata for hints about which generator to use. The `\x78\x9C` bytes are the deflate magic — a reliable indicator of zlib-compressed content.
...
**关键结论：** PDF xref generation numbers are rarely validated by viewers, making them a low-noise steganographic channel. Any value other than 0 (live) or 65535 (deleted) is suspicious. Use `pdf-parser.py --raw` to inspect raw xref entries without parser normalization.
...
**关键结论：** ANSI escape sequences encode visual information only revealed by terminal rendering. Always try `more` or `less -r` if content looks like terminal output. Cursor-positioning sequences can spell out text that only appears correct on a terminal.
...
**关键结论：** ECB-mode pixel encryption leaks structure via identical ciphertexts for identical plaintext pixels. With only 256 possible grayscale values, the full lookup table is trivial to precompute. The encrypted image will show the same shapes/edges as the original — recognizable structure confirms ECB mode.
...
**关键结论：** QR codes are strictly binary — any multi-color image that "looks like" a QR is hiding a 2^N coloring. Because QR has heavy Reed-Solomon error correction, multiple partitions can decode (each carries a different message in the same physical grid). Always try all 2^N mappings; with N<=8 the brute force is negligible and `zbarimg` filters the valid ones automatically.

**参考：** STEM CTF: Cyber Challenge 2019 — QvR Code, writeup 13375

---

## RTF Ignorable-Destination Custom-Tag Extraction (VolgaCTF 2013)

RTF destinations beginning with `\*` tell a conforming reader that an unknown destination may be ignored. Challenge authors can place ordered payload fragments in custom control groups: the document renders normally while the raw RTF contains the hidden data.

```python
import base64
import re

rtf = open('document.rtf', encoding='latin-1').read()

# Example groups: {\*\volgactf12 BASE64_FRAGMENT}
blocks = re.findall(r'\{\\\*\\volgactf(\d+)\s+([^}]+)\}', rtf)
blocks.sort(key=lambda item: int(item[0]))
encoded = ''.join(fragment.strip() for _, fragment in blocks)
payload = base64.b64decode(encoded, validate=True)
open('hidden.bin', 'wb').write(payload)
```

Search the raw file rather than the rendered page:

```bash
grep -oP '\\\*\\[A-Za-z]+[0-9]*' document.rtf | sort -u
```

Preserve backslashes, braces, whitespace and numeric suffixes when copying evidence. A custom destination is not automatically malicious, but indexed unknown tags plus high-entropy content are strong signals; sort numerically rather than lexicographically before concatenating fragments.
