# CTF Forensics - Image Bitplane, QR and JPEG Stego
...
- [QR Code Tile Reconstruction (UTCTF 2026)](#qr-code-tile-reconstruction-utctf-2026)
  - [QR Version 1 Format-String Pruning (Square CTF 2018)](#qr-version-1-format-string-pruning-square-ctf-2018)
...
- [Binary-Grid Text Rendering to QR (Pragyan CTF 2019)](#binary-grid-text-rendering-to-qr-pragyan-ctf-2019)

---

## JPEG Unused Quantization Table LSB Steganography (EHAX 2026)

**模式（Jpeg Soul）：** "Insignificant" hint points to least significant bits in JPEG quantization tables (DQT). JPEG can embed DQT tables (ID 2, 3) that are never referenced by frame markers — invisible to renderers but carry hidden data.
...
**关键结论：** JPEG quantization tables are metadata — they survive recompression and most image processing. Unused table IDs (2-15) can carry arbitrary data without affecting the image.

---

## BMP Bitplane QR Code Extraction + Steghide (BYPASS CTF 2025)

**模式（Gold Challenge）：** BMP image with QR code hidden in a specific bitplane. Extract the QR code to obtain a steghide password.
...
**关键结论：** Standard LSB (least significant bit) tools check bit 0 only. Hidden QR codes may be in bit 1 or bit 2 — always check multiple bitplanes systematically. BMP format preserves exact pixel values (no compression artifacts).

---

## Image Jigsaw Puzzle Reassembly via Edge Matching (BYPASS CTF 2025)

**模式（Jigsaw Puzzle）：** Archive containing multiple puzzle piece images that must be reassembled into the original image. Reassembled image contains the flag (possibly ROT13 encoded).
...
**关键结论：** Edge-matching works by minimizing pixel differences at shared borders. The greedy approach (place piece with smallest total edge difference to already-placed neighbors) works well for most CTF puzzles. For harder puzzles, add backtracking.

---

## F5 JPEG DCT Coefficient Ratio Detection (ApoorvCTF 2026)

**模式（Engraver's Fault）：** Detect F5 steganography in JPEG images by analyzing DCT coefficient distributions. F5 decrements ±1 AC coefficients toward 0, creating a measurable ratio shift.
...
**关键结论：** F5 steganography shifts ±1 coefficients toward 0, reducing the ±1/±2 ratio. Natural JPEGs have ratio 0.25-0.45; F5-modified drop below 0.10. Sparse images (mostly flat/white) need the secondary ±2/±3 metric because their ±1 counts are inherently low.

---

## PNG Unused Palette Entry Steganography (ApoorvCTF 2026)

**模式（The Gotham Files）：** Paletted PNG (8-bit indexed color) hides data in palette entries that no pixel references. The image uses indices 0-199 but the PLTE chunk has 256 entries — indices 200-255 contain hidden ASCII in their red channel values.
...
**关键结论：** QR codes have structural constraints (finder patterns, timing patterns, format info) that drastically reduce the search space. Use QR structure as anchors before brute-forcing tile positions.

### QR Version 1 Format-String Pruning (Square CTF 2018)

当二维码被切成行列条带或小块时，不要先枚举全部排列。Version 1 QR 的 format information 只来自 8 个纠错级别/掩码组合，共 32 个合法 15-bit 串；第 8 行和第 8 列上的已知位可以先筛掉不可能的条带，再枚举剩余顺序。

```python
valid_formats = load_32_valid_qr_formats()

anchored = []
for strip in strips:
    observed = extract_known_format_positions(strip)
    if any(matches_known_positions(observed, fmt) for fmt in valid_formats):
        anchored.append(strip)

for order in constrained_permutations(strips, anchored):
    candidate = assemble_qr(order)
    if decode_qr(candidate):
        break
```

先用 finder、timing 和 format bit 建硬约束，再用解码器/ECC 作为最终 oracle。若题目只给出部分第 8 行/列，也可以把它们与 32 个合法 format string 的对应位置逐一比对，用局部一致性剪枝。

**关键结论：** QR 格式本身就是排列恢复约束；先枚举 32 个合法 format string，比对所有条带排列更稳、更快。
...
**模式（Lost Signal）：** Image with randomized pixel colors hides a QR code. Pixels are visited in a seed-determined permutation order, and data is interleaved across multiple bitplanes of the luminance (Y) channel.
...
**关键结论：** The seed defines a deterministic pixel visit order (Fisher-Yates shuffle via `RandomState`). Without the correct seed, output is random noise. Bits from different bitplanes are interleaved (bit 0 from pixel N, bit 1 from pixel N, bit 0 from pixel N+1, ...), doubling the data density. Try the Y (luminance) channel first — it has the highest contrast for hidden binary data.
...
**关键结论：** Extract thumbnails with `exiftool -b -ThumbnailImage`. Dark pixels act as a selection mask over the photographed text. Use OCR (ABBYY FineReader, Tesseract) to get the text grid, then map dark thumbnail pixels to character positions.
...
**关键结论：** When standard `zsteg`/`stegsolve` find nothing, try filtering pixels by value range before LSB extraction. The carrier pixels may be restricted to near-black, near-white, or specific color ranges.
...
**关键结论：** Most image editors and viewers crop to the stated dimensions, hiding the padding. Use `jpegtran -crop` or raw DCT decoders to access full block data.
...
**关键结论：** Nearest-neighbor interpolation selects exact pixel values (no blending), preserving the hidden data. Bilinear or bicubic interpolation would average surrounding pixels, destroying the message. The challenge name or description often hints at the interpolation method.
...
**关键结论：** Unlike LSB (Least Significant Bit) stego (single channel, single bit), parity stego uses the combined sum of all channels. Look for challenge hints about "pairs", "couples", or "adding colors".
...
**关键结论：** Linked-list pixel traversal hides both the message and the reading order. Standard LSB analysis misses this because only specific pixels carry data. Look for images where green/blue channels have suspiciously structured values (small numbers that could be coordinates).
...
**关键结论：** Frame differential steganography hides data in the temporal domain rather than spatial. Standard image stego tools analyze single frames and miss inter-frame changes. Extract all frames, then diff consecutive pairs looking for single-pixel-value increments.
...
**关键结论：** For small files (< 10KB), the total search space for single-bit flips is `8 * file_size` — typically under 80,000 candidates, easily brute-forceable. Use thumbnail generation as a fast validity check (corrupt JPEGs fail to decode), then OCR on survivors. JPEG compressed data rule: `0xFF` is always followed by `0x00` (stuffed byte) or a marker — violations indicate the corruption location.
...
**关键结论：** CTF jigsaw challenges rarely require manual work. Carve pieces, stitch, run `gaps` — it uses a genetic algorithm to reassemble in minutes. Read `exiftool` on each piece for the size hint.
...
**关键结论：** Always scan the first ~256 bytes of a JPEG with `xxd`/`hexdump -C` for ASCII runs — authors sometimes stuff passphrases into reserved areas of JFIF/APPn segments where `exiftool` doesn't surface them, but they're trivially visible in a hex view. Pair the leak with `steghide`, `outguess`, or `stegseek` wordlist seeding.

**参考：** Quals Saudi and Oman National Cyber Security CTF 2019 — Hack a nice day, writeup 13232
...
**关键结论：** PNG has two orthogonal parseability gates: the 8-byte signature and the case of each chunk name (first letter uppercase = critical). Fix both before concluding the file is empty. `pngcheck -v` flags exactly which byte/chunk is wrong. Once readable, treat EXIF `Artist`, `Description`, and `tEXt`/`iTXt` chunks as prime hiding spots.

**参考：** Pragyan CTF 2019 — Magic PNGs, writeup 13833

---

## Binary-Grid Text Rendering to QR (Pragyan CTF 2019)

A text file containing only `0`, `1` and whitespace may be a serialized image rather than a binary byte stream. Remove separators, verify that the bit count forms a square, render one module per bit at an enlarged scale, and test both polarity choices with a barcode decoder.

```python
import math
from PIL import Image, ImageDraw

bits = ''.join(ch for ch in open('01qr').read() if ch in '01')
side = math.isqrt(len(bits))
if side * side != len(bits):
    raise ValueError(f'bit count {len(bits)} is not a square')

def render_qr(zero_is_black, output, scale=6, quiet=4):
    canvas_side = (side + 2 * quiet) * scale
    image = Image.new('L', (canvas_side, canvas_side), 255)
    draw = ImageDraw.Draw(image)
    for row in range(side):
        for col in range(side):
            bit = bits[row * side + col]
            black = (bit == '0') if zero_is_black else (bit == '1')
            if black:
                x = (col + quiet) * scale
                y = (row + quiet) * scale
...
render_qr(True, 'qr-zero-black.png')
render_qr(False, 'qr-one-black.png')
```

```bash
zbarimg qr-zero-black.png qr-one-black.png
```

The quiet zone and integer scaling keep module boundaries sharp. If neither polarity decodes, test row/column transposition, reversal and rotation before treating the grid as raw ASCII bits. In the Pragyan `EXORcism` variant the QR payload is hexadecimal repeating-key XOR ciphertext; recover the key from a confirmed flag prefix using [Repeating-Key XOR Recovery from a Known Prefix](../crypto/classical-xor-and-substitution-ciphers.md#repeating-key-xor-recovery-from-a-known-prefix), rather than guessing `flag` or an event name.
...
- **APNG frame extraction:** Animated PNG has multiple frames; extract with `apngdis` or parse `fdAT`/`fcTL` chunks. See pdf-png-gif-and-text-stego.md.
- **PNG height/CRC manipulation:** Modify IHDR height field, brute-force until CRC matches to reveal hidden rows. See pdf-png-gif-and-text-stego.md.
- **Pixel coordinate chain stego:** Linked-list traversal where R=data byte, G/B=next pixel coordinates. See image-bitplane-qr-and-jpeg-stego.md.
- **AVI frame differential:** XOR consecutive video frames to reveal hidden data in pixel differences. See image-bitplane-qr-and-jpeg-stego.md.
...
See pdf-png-gif-and-text-stego.md, audio-frequency-and-archive-stego.md, and video-document-and-media-stego.md for full code examples and decoding workflows.
