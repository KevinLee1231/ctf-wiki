...
数据清洗 Misc 题。表格中的姓名、手机号、身份证号、余额被多种格式和字符混淆污染，需要按审计规则做 Unicode 归一化、去零宽字符、手机号白名单校验、身份证校验位检查和正余额统计。
...
手机号：以 13912345678 为例，表格中存在 `13912345678`、`139-1234-5678`、`139 1234 5678`、`+8613912345678`、`(+86)139-12345678`、`139(1234)5678` 六种格式，并随机混入全角字符和零宽字符；清洗后还要按规则筛选白名单号段。

身份证号：随机替换最后一位或增长一位，并随机插入零宽字符，需要恢复后校验 18 位身份证校验位。

余额：存在 `123`、`￥123`、`123CNY` 三种格式，只统计符合条件用户的正数余额。
...
def main():
    ...
    try:
        ...
        prefix_rule = rules_df.loc[rules_df['Config_Item'] == 'Allow_Prefix',
'Value'].values[0]
    ...
    dedup_count = len(df)
    print(f"[-] 数据量变化: {original_count} -> {dedup_count} (剔除重复:
{original_count - dedup_count})")
    df['clean_phone'] = df['Phone'].apply(lambda x: extract_valid_phone(x,
valid_prefixes))
    ...
    target_users['clean_balance'] =
target_users['Balance'].apply(parse_balance)
    q4_ans = target_users[target_users['clean_balance'] > 0]
['clean_balance'].sum()
...
- 核心技巧：脏数据标准化、字段校验和规则过滤。
- 识别信号：CSV/XLSX 中同一字段出现全角、零宽字符、多种格式和规则表时，应先统一规范化再做业务校验。
- 复用要点：把清洗、去重、校验、统计拆成独立函数，避免直接对原始字符串做判断。
