...
题目附件是 Windows 程序 `BabyDriver.exe`，运行时会释放并加载一个驱动，再通过用户态到内核态的通信完成 flag 校验。关键机制是：用户态先对 32 字节 flag 做逐字节异或下标处理，再通过 `NtQueryInformationFile` 触发驱动侧回调；驱动中通过 `ExRegisterAttributeInformationCallback` 注册的回调函数参与校验，并暴露出 AES 加密特征。解题需要同时分析三环程序和释放出的驱动。

## 解题过程

首先查看字符串，发现 `Please Input Your Flag`，交叉引用定位到主函数。

### 分析sub_140006380

`sub_1400035A0` 是随机产生 8 位字符的函数，`sub_1400036C0` 通过前面生成的 `FileName` 将驱动释放到固定位置，`sub_1400037A0` 和 `sub_140003890` 是驱动加载启动相关操作，同时判断程序启动环境是否为 Win7 x64。
...
发现 AES 特征，怀疑核心加密是 AES。这里不依赖特定外部文章的实现细节，只需要使用标准 AES 逆运算恢复驱动侧处理结果；同时不要忘记三环程序在送入驱动前还有一个逐字节异或下标的预处理。
...
- 核心技巧：先从用户态程序定位输入长度、预处理和驱动释放行为，再在驱动中分析回调通信与 AES 校验逻辑。
- 识别信号：Reverse 附件名、题目名和程序行为都指向 driver，且用户态参数最终进入 `NtQueryInformationFile` 这类系统调用时，应检查是否借助内核回调或 hook 建立 R3/R0 通道。
- 复用要点：释放后自删除的驱动要在删除前动态提取；恢复 flag 时要按真实数据流逆序处理，先逆 AES，再还原用户态按下标 XOR 的变换。
