...
- [Comprehensive Bypass Strategies](#comprehensive-bypass-strategies)
  ...
  - [速查: Check to Bypass](#速查-check-to-bypass)
...
**Bypasses:**
```bash
# 1. LD_PRELOAD (see vm-obfuscation-and-transform-patterns.md for full hook)
...
**Bypasses:**
```bash
# 1. Frida hook (see frida-angr-lldb-and-x64dbg.md for clock_gettime hook)
...
// SIGSEGV handler that does real work (see vm-obfuscation-and-transform-patterns.md for MBA pattern)
...
Beyond basic switch-case (see vm-obfuscation-and-transform-patterns.md): modern OLLVM variants use:
...
更偏 CTF writeup 的 call-less chaining 案例已下沉到 signal-trace-and-packed-anti-analysis.md。
