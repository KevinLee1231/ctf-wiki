# CTF Reverse - Packers, Deobfuscation and Debug Automation
...
- 这是 **高级工具与去混淆** 的主卷：商业壳、diff、框架、仿真、符号执行。
- GDB 自动化、Ghidra 脚本、patching 和特定调试技巧也已并入本卷。
- 普通静态 / 动态分析不要从这里开始，先读 disassemblers-debuggers-and-basic-tools.md 或 frida-angr-lldb-and-x64dbg.md。
...
VMProtect virtualizes code into custom bytecode. Identify VM entry (pushad-like), find handler table (large indirect jump), trace handlers dynamically. For CTF, focus on tracing operations on input rather than full devirtualization. Themida: dump at OEP with ScyllaHide + Scylla. See packers-deobfuscation-and-debug-automation.md.
...
**关键结论：** Full devirtualization is rarely needed for CTF. Focus on tracing what operations are performed on your input. Hook comparison/crypto functions called from within the VM.
...
BinDiff and Diaphora compare two binaries to highlight changes. Essential when challenge provides patched/original versions. Export from IDA/Ghidra, diff to find vulnerability or hidden functionality. See packers-deobfuscation-and-debug-automation.md.
...
**关键结论：** LLVM's optimization passes (inlining, constant folding, dead code elimination) dramatically simplify lifted VM bytecode. A custom VM with 26 registers and 3 opcodes that produces 1300 lines of IL reduces to ~150 lines after `-O3`, revealing the underlying algorithm (e.g., Collatz sequence computation).

---

### Custom VM Bytecode Lifting to LLVM IR

Transpile custom VM bytecode to LLVM IR, then use `opt -O3` to simplify (inlining, constant folding, dead code elimination). Reduces 1300 lines to ~150 lines, revealing the underlying algorithm. See packers-deobfuscation-and-debug-automation.md.
## Advanced GDB Techniques

### Advanced GDB (pwndbg, rr)

pwndbg: `context`, `vmmap`, `search -s "flag{"`, `telescope $rsp`. GEF alternative. Reverse debugging with `rr record`/`rr replay` — step backward through execution. Python scripting for brute-force and automated tracing. See packers-deobfuscation-and-debug-automation.md.
...
**关键结论：** When a binary enforces linear arithmetic relationships between input bytes, ILP solvers directly find the satisfying assignment once constraints are extracted via GDB automation.
...
**关键结论：** Position-encoded input (`input[i]=i`) combined with zero flag monitoring reveals the full key/password in one pass — the zero flag fires when the expected value for position i equals i itself.
...
**关键结论：** Execute-only prevents file reading but not execution. LD_PRELOAD constructors run inside the process where `/proc/self/mem` provides access to mapped memory regardless of file permissions.
