...
**模式（Ms Blue Sky）：** Find target's posts on BlueSky social media.
...
- 先用 `sherlock username` 做 CLI 级首轮枚举，再用 [whatsmyname.app](https://whatsmyname.app) 与 [namechk.com](https://namechk.com) 交叉核对。`WhatsMyName` 当前更偏数据源与网页入口，注意平台误报。See social-media.md.
...
**模式（Massive-Equipment393）：** Reddit username -> Spotify social link -> Base58-encoded string -> Spotify playlist descriptions (base64) -> first-letter acrostic from song titles.
...
**关键结论：** Gaming profiles are often overlooked in OSINT but contain rich metadata (play times, real names, linked accounts, server regions). Guild/clan trackers index public game APIs and cache historical data. Character names are frequently reused across platforms.
...
**模式（Skybound Secrets）：** Flag hidden in a Bluesky post using Unicode homoglyph steganography — visually identical characters from different Unicode blocks encode binary data.
...
**模式（Where was Chine）：** Target's physical location identified through fitness tracking data. Username discovered on Twitter → alias found in GitHub code → alias searched on Strava → running route endpoint reveals location.
...
5. Verify with Google Maps user-submitted photos (see geolocation-and-media.md)

**关键结论：** Fitness apps are high-value OSINT targets because users rarely restrict activity visibility. A single public run reveals home/work neighborhoods. Cross-reference GPS endpoints with Google Maps to identify specific parks, buildings, or landmarks.

---

## Discord API Enumeration

**模式（Insanity 1 & 2, 0xFun 2026）：** Flags hidden in Discord server metadata not visible in normal UI.
