...
- [Web / DNS OSINT 速查技巧族：公开索引、历史快照与仓库痕迹](#web-dns-osint-速查技巧族公开索引历史快照与仓库痕迹)
  ...
  - [Google Docs/Sheets](#google-docssheets)
...
**关键结论：** The `itp:face` filter is especially useful for OSINT — it strips out logos, banners, and UI screenshots from results, leaving only profile photos. Combine with `site:` and date range (`after:YYYY-MM-DD`) for targeted reconnaissance.
...
**关键结论：** DNS TXT records are publicly queryable. Always check TXT, CNAME, MX for CTF domains and subdomains.
...
**模式（Rogue, VuwCTF 2025）：** Hidden information in GitHub repo comments (issue comments, PR reviews, commit messages, wiki edits).
...
**模式（Shell Game）：** Track organizational donors through FEC filings.
...
**关键结论：** SSH host keys are unique per server. If a hidden service runs SSH, its fingerprint can be searched on Shodan/Censys to find the real IP. This technique also works to de-anonymize services behind CloudFlare or other CDNs. Search both SSH fingerprints and TLS certificate fingerprints.
...
**模式（O-Syn-T）：** A port appears open on a standard service port (e.g., 22/SSH), but the service behind it is not what it claims. A basic SYN scan reports the port as open, but service version detection reveals a fake or custom banner containing the flag.
...
**关键结论：** Multi-challenge CTFs often leak infrastructure details cross-challenge. Always map the shared subnet first, then pivot info from the weakest challenge to the most constrained one.

**参考：** RITSEC CTF 2018 — Lazy Dev → Archivr, writeups 12234-12235

---

## Web / DNS OSINT 速查技巧族：公开索引、历史快照与仓库痕迹
...
资源入口已集中到 web-and-dns.md，这里仅保留跳转。
