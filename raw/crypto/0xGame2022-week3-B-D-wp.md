...
题目生成 2048 位 RSA 模数 $N=pq$，但私钥指数并非均匀随机，而是 `floor(N^(1/4))` 与一个 250 位随机数异或。随机扰动只改变低位，所以 $d$ 仍约为 $N^{0.25}$，显著小于正常 RSA 私钥，落入 Boneh–Durfee 小私钥攻击范围。公开参数为 $N,e,c$，目标是先恢复 $d$，再正常解密密文。

## 解题过程

由 $ed-k\varphi(N)=1$，平衡素数条件下可把未知量转成模 $e$ 的小根问题。Boneh–Durfee/Herrmann–May 方法构造多项式移位格，经 LLL 得到两个独立多项式，再由结式求出小根并恢复 $d$。理论界限为

$$
d<N^\delta,\qquad \delta<1-\frac{\sqrt2}{2}\approx0.292.
$$

本题取 `delta = 0.251`、`m = 4`，与生成逻辑中的 $d\approx N^{0.25}$ 相符。Wiener 攻击通常要求更强的小私钥界限，本实例没有必要把它当成可靠首选。下面保留完整 Sage 实现，避免原 PDF 只说“网上找脚本”却没有解释攻击条件。
...
决定攻击路线的是私钥规模而不是模数规模：生成式只扰动 $N^{1/4}$ 的低 250 位，使 $d$ 仍位于约 $N^{0.25}$ 的窄区间，满足 Boneh–Durfee 的小根界限。参数 `delta` 应略高于实际指数比例但保持低于约 0.292；`m` 增大通常提高成功率，也会显著增加格维度和 LLL 耗时。恢复 $d$ 后仍需执行 $m=c^d\bmod N$，原 PDF 只打印私钥而没有写完这一最后步骤。
