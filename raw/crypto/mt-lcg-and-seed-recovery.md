# CTF Crypto - MT, LCG and Seed Recovery

## 阅读定位

- 这是 PRNG 的 **主卷**：MT19937、LCG、time seed、基础 state recovery、key recovery 与常见同步问题都先从这里开始。
- 如果你已经确认题目属于 PRNG，但需要 2017+ 的特殊 CTF 变体、约束传播或更长尾案例，再读 prng-z3-lcg-and-timing-attacks.md。
...
**关键结论：** `random.random()` returns `(a*2^27+b)/2^53` where `a` = 27 bits from one MT output and `b` = 26 bits from the next. Truncating `int(float * 256)` yields only 8 bits per float, so 3360+ observations are needed (vs. 624 for integer outputs). The `not_random` library precomputes the GF(2) relationship between observed bits and state bits.
...
**适用条件：** Server uses Python's `random.random()` for security-sensitive tokens (session IDs, password resets, CSRF tokens) and also exposes random values through another endpoint. The `not_random` library handles the bit-level math — focus on collecting enough float observations and synchronizing timing with the target operation.
...
**关键结论：** Python's `random` and C's `rand()` are completely different PRNGs. When a C binary uses `srand(time(NULL))`, the only way to reproduce the sequence from Python is `ctypes.CDLL` calling the same libc's `srand`/`rand`. Load the challenge's provided `libc.so.6` for exact compatibility. This works for any C PRNG output prediction — XOR keys, random challenges, token generation, or encrypted heap data.
...
**模式（BitStorm）：** PRNG using only XOR, shifts, and rotations is linear over GF(2).

**关键结论：** Express entire PRNG as matrix multiplication: `output_bits = M * seed_bits (mod 2)`. With enough outputs, Gaussian elimination recovers the seed.
...
# Use GF(2) Gaussian elimination (see aes-modes-mac-and-hash-oracles.md solve_gf2)
...
**模式（numbers go brrr）：** Middle-square method with small seed space.
...
**模式（Totally Random Art）：** Flag bytes seed Python `random.Random()`. First N bytes of flag are known format, remaining bytes produce deterministic output.
...
**模式（Unorthodox IV）：** Server encrypts with one of N random modes/IVs per encryption. Can submit own plaintexts.
...
**关键结论：** Probe for mode reachability first to avoid wasting attempts. Elimination-based search is more efficient than confirmation-based when modes are randomized.

---

## RSA Key Reuse / Replay (UTCTF 2024)

**模式（simple signature）：** RSA keys reused across rounds with alternating inputs.
...
**模式（Chaotic Trust）：** Stream cipher using the logistic map `x_{n+1} = r * x * (1 - x)` as PRNG. Keystream generated by packing iterated float values.

**关键结论：** Logistic map with `r ≈ 4.0` is chaotic but deterministic — recovering the seed (initial x value) enables full keystream reconstruction. Seed is usually a decimal between 0 and 1, such as 0.123456789.
...
**关键结论：** The LIFO cache means observed values are in reverse generation order — reverse them with `tac` before solving. The Z3 `QF_BV` (quantifier-free bitvector) theory efficiently handles the bitwise operations. Typically 5-10 consecutive outputs suffice for a unique solution.
...
**适用条件：** Web challenge where JavaScript generates predictable-looking random values (tokens, verification codes, game rolls) using `Math.random()`. Look for patterns like `Math.floor(N * Math.random())` or `Math.random().toString(36).substr(2)` in client-side or server-side Node.js code.
