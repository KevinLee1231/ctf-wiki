# CTF Misc - VM, Z3, Sandbox and Game Basics

## 阅读定位

- 这是 games / VMs / constraint solving 系列的 **主卷**：WASM、Python、Z3、sandbox、基础混合题都从这里起步。
- 如果题目更偏游戏状态、Cookie / WebSocket 操纵或短链路 VM，再读 game-state-websocket-and-wasm.md。
- 如果题目已经是复杂交互题、容器、jail 或 solver 管线，再转 interactive-containers-jails-and-solvers.md。
...
**模式（Tac Tic Toe, Pragyan 2026）：** Game with unbeatable AI in WebAssembly. Proof/verification system validates moves but doesn't check optimality.

**关键结论：** If the proof generation depends only on move positions and seed (not on whether moves were optimal), patching the WASM to make the AI play badly produces a beatable game with valid proofs.
...
**模式（MazeRunna, 0xFun 2026）：** Roblox game where the flag is hidden in an older published version. Latest version contains a decoy flag.
...
**模式（CTFaaS, LACTF 2026）：** Container deployer with claimed ServiceAccount isolation.
...
**模式（Spare Me Some Change）：** Trading/economy games where large multipliers amplify tiny floating-point errors.

**关键结论：** When decimal values (0.01-0.99) are multiplied by large numbers (e.g., 1e15), floating-point representation errors create fractional remainders that can be exploited.
...
**模式（Chusembly）：** Web app with custom instruction set (LD, PUSH, PROP, CALL, IDX, etc.) running on a Python backend. Safety check only blocks the word "flag" in source code.

**关键结论：** `PROP` (property access) and `CALL` (function invocation) instructions allow traversing Python's MRO chain from any object to achieve RCE, similar to Jinja2 SSTI.
...
**关键结论：** Lua sandboxes typically filter specific function *calls* but not *table lookups*. Access blocked functions through table indexing (`os["execute"]`), string concatenation for function names, or alternate I/O libraries (`io.popen`). Also check if `loadstring` (Lua 5.1 alias for `load`) is unblocked.
...
**关键结论：** `TracePoint` operates at a lower level than `set_trace_func` in Ruby — C-call hooks fire before Ruby-level event hooks, allowing sandbox escape. Any subsequent C-method call (even benign ones) triggers the payload.
...
See vm-z3-sandbox-and-game-basics.md for YARA rules, type systems as constraints, boolean logic gate network SAT solving.
...
`close(0x8000000000000002)` passes 64-bit SECCOMP check (≠ 2) but kernel truncates to 32-bit (== 2), closing fd 2. Next `open()` returns fd 2 for arbitrary file. Type-width mismatch between BPF filter and kernel. See interactive-containers-jails-and-solvers.md.
