...
category: forensics
related_skill: ctf-forensics
original_path: "C:/Users/LMY/.agents/skills/ctf-forensics/references/pcap-protocol-and-credential-recovery.md"
original_reference: "references/pcap-protocol-and-credential-recovery.md"
original_sha256: "f220fc6b7d1a11a5f24fc1cb2c45ac64b3a7d72b94dc77d25a789483e1bb0dce"
archived_at: 2026-05-21
status: archived
---

# Archived Skill Reference: ctf-forensics/references/pcap-protocol-and-credential-recovery.md

> 本文件是 skill reference 的 raw archive 快照，用作 ctf-wiki 的原始资料。原 skill 文件仍保留在原位置，当前迁移阶段不修改原 skill。

## Archive Metadata

- Original path: `C:/Users/LMY/.agents/skills/ctf-forensics/references/pcap-protocol-and-credential-recovery.md`
- Original SHA256: `f220fc6b7d1a11a5f24fc1cb2c45ac64b3a7d72b94dc77d25a789483e1bb0dce`
- Archived at: `2026-05-21`

---
# CTF Forensics - PCAP Protocol and Credential Recovery

## Table of Contents
- [tcpdump 速查](#tcpdump-速查)
- [TLS/SSL Decryption via Keylog File](#tlsssl-decryption-via-keylog-file)
- [PCAP 基础证据技巧族：Wireshark、端口、设备与凭据](#pcap-基础证据技巧族wireshark端口设备与凭据)
...
- [HTTP File Upload Exfiltration in PCAP (MetaCTF 2026)](#http-file-upload-exfiltration-in-pcap-metactf-2026)
  - [HTTP Exfiltration in PCAP](#http-exfiltration-in-pcap)
...
- [Network Forensics 速查](#network-forensics-速查)
---
...
**关键结论：** Use tcpdump for quick command-line triage when Wireshark is unavailable. Pipe to `strings` or `grep` for fast flag hunting: `tcpdump -r capture.pcap -A | grep -i flag`.
...
**Method 3 — Weak RSA key factoring (see also linux-git-browser-and-container-forensics.md):**
```bash
...
**关键结论：** Look for keylog files (`.log`, `sslkeys.txt`) in challenge artifacts. If the challenge gives you a private key, use it directly. For weak RSA keys in certificates, factor the modulus to derive the private key.

---

## PCAP 基础证据技巧族：Wireshark、端口、设备与凭据
...
**模式（Gibberish）：** USB keyboard PCAP with simultaneous multi-key presses = stenography chording.
...
**模式（1.5x-engineer）：** "1.5x" hints at the encoding ratio.
...
**关键结论：** Challenge name often hints at encoding ratio or technique.
...
**关键结论：** Always start with `--export-objects` to extract transferred files before deep packet analysis. The flag is often in the exfiltrated file itself.

---

### HTTP Exfiltration in PCAP

**Quick path:** `tshark --export-objects http,/tmp/objects` extracts uploaded files instantly. Check for multipart POST uploads, unusual User-Agent strings, and exfiltrated files (images with flag text). See pcap-protocol-and-credential-recovery.md.
...
**关键结论：** OpenSSL stores `master_key[48]` directly before `session_id[32]` in `ssl_session_st`. Search the coredump for the session ID (from the TLS handshake), then read the 48 bytes before it. This works with coredumps, memory dumps, and Volatility memory extractions.
...
**关键结论：** When PCAP contains many same-sized file transfers, suspect a split archive. The fragment order is not the download order — look for an Apache/nginx directory listing page in the PCAP whose modification timestamps provide the correct reassembly sequence. The smallest file is the trailing fragment.
...
**关键结论：** WiFi CTF challenges often have multiple encryption key changes throughout the capture. Decrypt, look for hints to the next password in the decrypted traffic, then decrypt the next segment. Check Internet Printing Protocol (IPP) streams for job-name fields containing flags.
...
**关键结论：** Damaged PCAPs are common in forensics CTF challenges. Always try `pcapfix` first -- it handles most corruption automatically. For manual repair, the pcap header is 24 bytes: magic(4) + version(4) + timezone(4) + sigfigs(4) + snaplen(4) + linktype(4).
...
**关键结论：** SAP Dialog protocol's "encryption" is simple obfuscation easily reversed. Cain and Abel (Windows) has built-in SAP Dialog decryption. For Linux, use pysap or SAP Wireshark dissector plugins.
...
**关键结论：** DNS NOERROR vs NXDOMAIN acts as a binary oracle leaking one bit per query — applicable to any DNS-based covert channel. Each query tests whether a candidate prefix is correct, allowing O(n) reconstruction where n is the number of bits in the flag.

See also: network-covert-auth-and-reassembly.md for advanced network forensics techniques (packet interval timing encoding, USB HID mouse/pen drawing recovery, NTLMv2 hash cracking, TCP flag covert channels, DNS steganography, multi-layer PCAP with XOR, Brotli decompression bomb seam analysis, SMB RID recycling, Timeroasting MS-SNTP).


---

## Network Forensics 速查
...
- **WPA/WEP WiFi decryption:** `aircrack-ng -w wordlist capture.pcap` cracks WPA handshake; WEP cracked with enough IVs. See pcap-protocol-and-credential-recovery.md.
- **PCAP repair:** `pcapfix -d corrupted.pcap` repairs broken PCAP headers/checksums for Wireshark loading. See pcap-protocol-and-credential-recovery.md.
- **USB HID keyboard decoding:** Extract 8-byte HID reports from USB captures; byte 2 = keycode, byte 0 = modifiers (Shift). See network-covert-auth-and-reassembly.md.
- **dnscat2 reassembly:** Decode hex/base32 subdomain labels, strip 9-byte dnscat2 header, deduplicate retransmissions, reassemble payload. See network-covert-auth-and-reassembly.md.
- **USB keyboard LED exfiltration:** Host-to-device HID SET_REPORT packets toggle Caps Lock LED. Timing encodes Morse code. See network-covert-auth-and-reassembly.md.

See pcap-protocol-and-credential-recovery.md for SMB3 decryption, credential extraction, and linux-git-browser-and-container-forensics.md for full TLS/TFTP/USB workflows.
