# CTF Forensics - Windows Registry, Logs and Credentials
...
- [Windows 末端痕迹技巧族：日志清理、哈希与 WMI](#windows-末端痕迹技巧族日志清理哈希与-wmi)
  - [Log Analysis](#log-analysis)
  - [When Logs Are Cleared](#when-logs-are-cleared)
  - [Windows Password Hashes](#windows-password-hashes)
  - [WMI Persistence Analysis](#wmi-persistence-analysis)
---
...
See windows-registry-logs-and-credentials.md for full event ID tables, registry analysis, SAM parsing, USN journal, and anti-forensics detection.

- **NTFS Alternate Data Streams (ADS):** Hidden data attached to files via named NTFS streams. Invisible to `dir`/Explorer. Detect with `fls -r image.dd | grep ":"`, extract with `icat`. See windows-registry-logs-and-credentials.md.
...
**关键结论：** User profiles are only created on first interactive logon (RDP or console), not via WMI/wmiexec remote execution.
...
**关键结论：** `certutil` is commonly abused by malware for base64 decoding as a living-off-the-land technique. `UEsD` is the base64 encoding of ZIP magic bytes `PK\x03` — use it as a memory scanning signature to find in-transit ZIP archives before they are written to disk or after they are deleted.
...
**关键结论：** Before spending hours carving LSASS or walking process heaps, run `clipboard` — CTF challenges about "Silly Rick copied his password" always surface here. Combine with `cmdline`, `consoles`, and `filescan` for full user-activity reconstruction.

**参考：** OtterCTF 2018 — Silly Rick, writeup 12596
...
**关键结论：** Don't hunt with `strings` alone. The Volatility plugin suite has a plugin for every artifact: clipboard, mimikatz (plaintext), hashdump (hashes), printkey (registry), memdump (per-process memory), netscan (sockets), pstree (process hierarchy), dlllist (loaded modules). Run them in order from cheapest to most expensive.

**参考：** OtterCTF 2018 — multiple challenges, writeups 12569–12572, 12596


---

## Windows 末端痕迹技巧族：日志清理、哈希与 WMI
...
See linux-git-browser-and-container-forensics.md for Linux attack chain analysis and Docker image forensics.
...
See windows-registry-logs-and-credentials.md for detailed parsing code and anti-forensics detection checklist.
...
See windows-registry-logs-and-credentials.md for SAM details and network-covert-auth-and-reassembly.md for NTLMv2 cracking from PCAP.
...
**模式（Backchimney）：** Malware uses WMI event subscriptions for persistence (MITRE T1546.003).
...
See windows-registry-logs-and-credentials.md for WMI repository analysis details.
