...
- 需要用最小命令建立事实：`file`、`strings`、`readelf`、`objdump`、`checksec`、GDB 断 `main`，或通过 IDA Pro MCP/r2 建立交叉引用。
...
| 结构恢复、交叉引用、伪代码和类型推断 | IDA Pro MCP（`idalib`） | 普通 native binary 的首选；按 `idb_list` → `idb_open` → `survey_binary` 建立全局画像，再围绕候选函数反编译。 |
...
| IDA 不可用或需要现有项目/raw language 导入 | Ghidra MCP / headless Ghidra | 普通静态分析备选；不是默认首选。遇到 SMC、packed code 或运行时生成逻辑时仍需配合动态 dump。 |
...
2. 普通 native binary 用 IDA Pro MCP survey 找候选校验点；再用动态调试确认输入如何影响分支。
...
| Raw WP | 可复用联系 |
|---|---|
| [西湖论剑2023-Dual-Personality-wp](../raw/reverse/西湖论剑2023-Dual-Personality-wp.md) | PE32 运行时 patch far jump 到 WoW64 `0x33` 代码段；必须按 64 位模式重反汇编后再逆 rolling add/xor。 |
